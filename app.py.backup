...
def get_user(tg_id: int):
    try:
        with closing(db()) as conn:
            ...
            if not row:
                return {"status": "ok", "userId": int(tg_id), "taps_available": 0, "tap_reward": 0.0, "earn_cap_remaining": 0.0, "taps_total": 0, "balance_usdt": 0.0, "bonus_given": 0}
...
def tap(data: dict):
    ...
    with closing(db()) as conn:
        ...
        new_balance = float(row["balance_usdt"]) + reward
        new_taps = taps_available - 1
        new_total = int(row["taps_total"]) + 1
...
# Оставляем твои платежные роуты без изменений
